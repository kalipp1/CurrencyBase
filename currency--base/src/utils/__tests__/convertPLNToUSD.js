...
describe('ConvertPLNtoUSD', () => {
  ...
  it('should return NaN when input is string', () => {
    ...
    expect(convertPLNToUSD('1223')).toBeNaN();
    expect(convertPLNToUSD('dassa')).toBeNaN();
  });
  it('should return NaN when input is empty', () => {
    expect(convertPLNToUSD()).toBeNaN();
  });
  it('should return "Error" when input is not a text or either value', () => {
    expect(convertPLNToUSD(null)).toBe('Error');
    expect(convertPLNToUSD({})).toBe('Error');
    expect(convertPLNToUSD([])).toBe('Error');
    expect(convertPLNToUSD(function() {})).toBe('Error');
  });
  it('should return "$0.00" when input is under 0', () => {
